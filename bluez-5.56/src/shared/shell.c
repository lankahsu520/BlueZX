...
#ifdef BLUEZX
static char *cmplt_repeater = "repeater";
#endif
...
static void cmd_menu(int argc, char *argv[])
{
	...
	if (argc < 2 || !strlen(argv[1])) {
		BT_SHELL_DBG("Missing name argument\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	menu = find_menu(argv[1], strlen(argv[1]));
	if (!menu) {
		BT_SHELL_DBG("Unable find menu with name: %s\n", argv[1]);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
static void cmd_back(int argc, char *argv[])
{
	if (data.menu == data.main) {
		BT_SHELL_DBG("Already on main menu\n");
		return;
	}
...
#ifdef BLUEZX
int shell_exec_helper(int argc, char *argv[], void *user_data)
{
	int err;
...
static int shell_exec(int argc, char *argv[])
{
	...
	ARGC_AND_ARGV_DB_DUMP(argc, argv);
...
static void rl_init_history(void)
{
	...
	using_history();
	DBG_TR_LN("call bt_shell_set_env ... (name: HISTORY, value: %s)", data.history);
...
void bt_shell_init(int argc, char **argv, const struct bt_shell_opt *opt)
{
...
#ifdef BLUEZX
		snprintf(optstr, sizeof(optstr), "+mrhvt:%s", opt->optstr);
#else
	...
	{
#ifdef BLUEZX
		snprintf(optstr, sizeof(optstr), "+mrhvt:");
#else
...
#ifdef BLUEZX
	if ( (argc>=2) && (SAFE_STRCMP(argv[1], "--repeater") ==0) )
	{
		data.repeater = 1;
		data.argc = 1;
		data.argv = &cmplt_repeater;
		data.mode = 1;
		goto done;
	}
#endif
	...
	DBG_TR_LN("call bt_shell_set_env ... (name: SHELL, data.name: %s, optind: %d)", data.name, optind);
	...
	DBG_IF_LN("(data.argc: %d, data.init: %d, data.mode: %d, data.monitor: %d)", data.argc, data.init, data.mode, data.monitor);
	if (data.mode)
	{
		DBG_TR_LN("call bt_shell_set_env ... (name: NON_INTERACTIVE, data.mode: %d)", data.mode);
...
{
	DBG_WN_LN("call mainloop_exit_success/mainloop_exit_failure ... (status: %d)", status);
...
void bt_shell_noninteractive_quit(int status)
{
	...
	DBG_WN_LN("call bt_shell_quit ... (status: %d, data.mode: %d, data.timeout: %d)", status, data.mode, data.timeout);
...
#ifdef BLUEZX
	DBG_WN_LN("(data.repeater: %d)", data.repeater);
	if (data.repeater)
	{
		return false;
	}
