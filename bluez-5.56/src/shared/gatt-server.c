					...
					void *user_data)
{
	...
	DBG_DB_LN("(opcode: %d, length: %d)", opcode, length);
								...
								void *user_data)
{
	...
	DBG_DB_LN("(op->opcode: %d, err: %d)", op->opcode, err);
					...
					void *user_data)
{
	...
	DBG_DB_LN("(data->opcode: %d, err: %d)", data->opcode, err);
	if (err != 0) {
		ecode = err;
		goto error;
	}
								...
								void *user_data)
{
	...
	DBG_DB_LN("(pwcd->length: %d, err: %d)", pwcd->length, err);
