				...
				GDBusDestroyFunction destroy)
{
	...
	for (method = methods; method && method->name; method++) {
		if (!check_experimental(method->flags,
					G_DBUS_METHOD_FLAG_EXPERIMENTAL))
		{
			DBG_IF_LN("=>> (name: %s, method->name: %s)", name, method->name);
			goto done;
		}
	}

	for (signal = signals; signal && signal->name; signal++) {
		DBG_IF_LN("=>> (signal->name: %s)", signal->name);
		if (!check_experimental(signal->flags,
					G_DBUS_SIGNAL_FLAG_EXPERIMENTAL))
		{
			DBG_IF_LN("=>> (name: %s, signal->name: %s)", name, signal->name);
			goto done;
		}
	}

	for (property = properties; property && property->name; property++) {
		DBG_IF_LN("=>> (property->name: %s)", property->name);
		if (!check_experimental(property->flags,
					G_DBUS_PROPERTY_FLAG_EXPERIMENTAL))
		{
			DBG_IF_LN("=>> (name: %s, property->name: %s)", name, property->name);
			goto done;
		}
	}
							...
							const char *path)
{
	...
	DBG_DB_LN("=>> (data->path: %s, data->introspect: %s)", data->path, data->introspect);
	...
	DBG_TR_LN("call invalidate_parent_data ... (path: %s)", path);
...
gboolean g_dbus_attach_object_manager(DBusConnection *connection)
{
	...
	DBG_TR_LN("call add_interface ... (data: %p)", data);
