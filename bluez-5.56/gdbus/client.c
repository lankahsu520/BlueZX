						...
						const char *interface)
{
	...
	for (list = g_list_first(client->proxy_list); list;
		...
		if (g_str_equal(proxy->interface, interface) == TRUE &&
			...
			DBG_WN_LN("(interface: %s)", interface);
			client->proxy_list =
...
static void get_managed_objects(GDBusClient *client)
{
	...
	DBG_WN_LN("(client->connected: %d)", client->connected);
	if (!client->connected)
		return;
					...
					DBusMessage *message, void *user_data)
{
	...
	DBG_TR_LN("call client->signal_func ... (client->signal_func: %p)", client->signal_func);
							...
							const char *root_path)
{
	...
	DBG_IF_LN("=>> (connection: %p, service: %s, path: %s, root_path: %s)", connection, service, path, root_path);
	...
	DBG_TR_LN("call g_dbus_add_service_watch ...");
	...
	DBG_TR_LN("call g_dbus_add_signal_watch ... (InterfacesAdded)");
	client->added_watch = g_dbus_add_signal_watch(connection, service,
						...
						interfaces_added,
						client, NULL);
	DBG_TR_LN("call g_dbus_add_signal_watch ... (InterfacesRemoved)");
					...
					void *user_data)
{
	...
	DBG_TR_LN("call get_managed_objects ... (proxy_added: %p, proxy_removed: %p, property_changed: %p)", proxy_added, proxy_removed, property_changed);
