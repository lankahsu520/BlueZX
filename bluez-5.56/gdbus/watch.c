					...
					const char *argument)
{
	...
	DBG_DB_LN("=>> (sender: %s, path: %s, interface: %s, member: %s, argument: %s)", sender, path, interface, member, argument);
...
static gboolean update_service(void *user_data)
{
	...
	DBG_TR_LN("call cb->conn_func ... (cb->conn_func: %p)", cb->conn_func);
	if (cb->conn_func)
...
static void service_reply(DBusPendingCall *call, void *user_data)
{
	...
	DBG_TR_LN("call dbus_pending_call_steal_reply ...");
					...
					struct filter_callback *callback)
{
	...
	DBG_IF_LN("=>> call dbus_message_new_method_call ... (data->name: %s, GetNameOwner)", data->name);
				...
				void *user_data, GDBusDestroyFunction destroy)
{
	...
	DBG_DB_LN("call filter_data_get ... (NameOwnerChanged)");
				...
				GDBusDestroyFunction destroy)
{
	...
	DBG_DB_LN("call filter_data_get ... (%s)", member);
				...
				GDBusDestroyFunction destroy)
{
	...
	DBG_DB_LN("call filter_data_get ... (PropertiesChanged)");
	data = filter_data_get(connection, signal_filter, sender, path,
