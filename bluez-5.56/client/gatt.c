...
struct chrc *bluezx_gatt_read_chrc(char *service_uuid, char *chrc_uuid)
{
	struct chrc *chrc_found = NULL;

	{
		GList *local_services = bluezx_gatt_local_services_get();
		if (local_services)
		{
			GList *l;
			for (l = local_services; l; l = g_list_next(l))
			{
				...
				{
					GList *local_chrcs = service->chrcs;
					GList *l;

					for (l = local_chrcs; l; l = g_list_next(l))
					{
						struct chrc *chrc = l->data;

						if ( (service_uuid) && (chrc_uuid) )
						{
							if ( (SAFE_STRCMP(service->uuid, service_uuid) == 0) && (SAFE_STRCMP(chrc->uuid, chrc_uuid) == 0) )
							{
								chrc_found = chrc;
								DBG_IF_LN("(service->uuid: %s, chrc->uuid: %s, chrc->value_len: %zd)", service->uuid, chrc->uuid, chrc->value_len);
								bt_shell_hexdump(chrc->value, chrc->value_len);

								char *startptr = qbuf_buff(&chrc->qbuf);
								size_t total = qbuf_total(&chrc->qbuf);
								DBG_IF_LN("(service->uuid: %s, chrc->uuid: %s, qbuf_total: %zd)", service->uuid, chrc->uuid, total);
								bt_shell_hexdump((const unsigned char *)startptr, total);
								goto found;
							}
						}
						else
...
static void print_service(struct service *service, const char *description)
{
	...
	if (!text)
		BT_SHELL_DBG("%s%s%s%s Service (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static void print_inc_service(struct service *service, const char *description)
{
	...
	text = bt_uuidstr_to_str(service->uuid);
	if (!text)
		BT_SHELL_DBG("%s%s%s%s Included Service (Handle 0x%04x)\n\t"
					"%s\n\t%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
void gatt_add_service(GDBusProxy *proxy)
{
	...
	DBG_TR_LN("call print_service_proxy ... ["COLORED_NEW"]");
...
#ifdef BLUEZX
#define PRINT_CHRC_DBG(s1,s2) \
	({ \
		struct chrc *chrc1 = (struct chrc *)s1; \
		const char *description1 = s2; \
		const char *text = bt_uuidstr_to_str(chrc1->uuid); \
		if (!text) BT_SHELL_DBG("%s%s%sCharacteristic (Handle 0x%04x)\n\t%s\n\t%s\n", description1 ? "[" : "", description1 ? : "", description1 ? "] " : "", chrc1->handle, chrc1->path, chrc1->uuid); \
		else BT_SHELL_DBG("%s%s%sCharacteristic (Handle 0x%04x)\n\t%s\n\t%s\n\t%s\n", description1 ? "[" : "", description1 ? : "", description1 ? "] " : "", chrc1->handle, chrc1->path, chrc1->uuid, text); \
	})
#else
static void print_chrc(struct chrc *chrc, const char *description)
{
	...
	if (!text)
		BT_SHELL_DBG("%s%s%sCharacteristic (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static void print_characteristic(GDBusProxy *proxy, const char *description)
{
	...
	PRINT_CHRC_DBG(&chrc, description);
}
...
void gatt_add_characteristic(GDBusProxy *proxy)
{
	...
	DBG_TR_LN("call print_characteristic ... ["COLORED_NEW"]");
...
static void print_desc(struct desc *desc, const char *description)
{
	...
	if (!text)
		BT_SHELL_DBG("%s%s%sDescriptor (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
void gatt_add_descriptor(GDBusProxy *proxy)
{
	...
	DBG_TR_LN("call print_descriptor ... ["COLORED_NEW"]");
...
static void read_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_ARRAY) {
		BT_SHELL_DBG("Invalid response to read\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (len < 0) {
		BT_SHELL_DBG("Unable to parse value\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
static void read_attribute(GDBusProxy *proxy, uint16_t offset)
{
	...
	BT_SHELL_DBG("Attempting to read %s\n", g_dbus_proxy_get_path(proxy));
}
...
void gatt_read_attribute(GDBusProxy *proxy, int argc, char *argv[])
{
	...
	BT_SHELL_DBG("Unable to read attribute %s\n",
						g_dbus_proxy_get_path(proxy));
	return bt_shell_noninteractive_quit(EXIT_FAILURE);
}
...
static int sock_send(struct io *io, struct iovec *iov, size_t iovlen)
{
	...
	if (ret < 0) {
		ret = -errno;
		BT_SHELL_DBG("sendmsg: %s", strerror(-ret));
	}
				...
				struct write_attribute_data *data)
{
	...
	if (proxy == write_io.proxy &&
		...
		BT_SHELL_DBG("Attempting to write fd %d\n",
						io_get_fd(write_io.io));
		if (sock_send(write_io.io, &data->iov, 1) < 0) {
			BT_SHELL_DBG("Failed to write: %s", strerror(errno));
			return bt_shell_noninteractive_quit(EXIT_FAILURE);
		}
		return;
	}
	...
	BT_SHELL_DBG("Attempting to write %s\n",
					g_dbus_proxy_get_path(proxy));
}
...
static uint8_t *str2bytearray(char *arg, size_t *val_len)
{
	...
	for (i = 0; (entry = strsep(&arg, " \t")) != NULL; i++) {
		...
		if (i >= G_N_ELEMENTS(value)) {
			BT_SHELL_DBG("Too much data\n");
			return NULL;
		}

		val = strtol(entry, &endptr, 0);
		if (!endptr || *endptr != '\0' || val > UINT8_MAX) {
			BT_SHELL_DBG("Invalid value at index %d\n", i);
			return NULL;
		}
...
void gatt_write_attribute(GDBusProxy *proxy, int argc, char *argv[])
{
	...
	BT_SHELL_DBG("Unable to write attribute %s\n",
						g_dbus_proxy_get_path(proxy));
...
static bool sock_read(struct io *io, void *user_data)
{
	...
	if (bytes_read < 0) {
		BT_SHELL_DBG("recvmsg: %s", strerror(errno));
		return false;
	}
	...
	if (chrc)
		BT_SHELL_DBG("[" COLORED_CHG "] Attribute %s (%s) "
				"written:\n", chrc->path,
				bt_uuidstr_to_str(chrc->uuid));
	else
		BT_SHELL_DBG("[" COLORED_CHG "] %s Notification:\n",
...
static bool sock_hup(struct io *io, void *user_data)
{
	...
	if (chrc) {
		BT_SHELL_DBG("Attribute %s %s sock closed\n", chrc->path,
...
static void acquire_write_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to acquire write: %s\n", error.name);
		dbus_error_free(&error);
		write_io.proxy = NULL;
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if ((dbus_message_get_args(message, NULL, DBUS_TYPE_UNIX_FD, &fd,
		...
		BT_SHELL_DBG("Invalid AcquireWrite response\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	BT_SHELL_DBG("AcquireWrite success: fd %d MTU %u\n", fd,
								write_io.mtu);
...
void gatt_acquire_write(GDBusProxy *proxy, const char *arg)
{
	...
	iface = g_dbus_proxy_get_interface(proxy);
	if (strcmp(iface, "org.bluez.GattCharacteristic1")) {
		BT_SHELL_DBG("Unable to acquire write: %s not a"
				" characteristic\n",
				g_dbus_proxy_get_path(proxy));
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
void gatt_release_write(GDBusProxy *proxy, const char *arg)
{
	if (proxy != write_io.proxy || !write_io.io) {
		BT_SHELL_DBG("Write not acquired\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
static void acquire_notify_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to acquire notify: %s\n", error.name);
		dbus_error_free(&error);
		write_io.proxy = NULL;
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if ((dbus_message_get_args(message, NULL, DBUS_TYPE_UNIX_FD, &fd,
					...
					DBUS_TYPE_INVALID) == false)) {
		BT_SHELL_DBG("Invalid AcquireNotify response\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	BT_SHELL_DBG("AcquireNotify success: fd %d MTU %u\n", fd,
								notify_io.mtu);
...
void gatt_acquire_notify(GDBusProxy *proxy, const char *arg)
{
	...
	iface = g_dbus_proxy_get_interface(proxy);
	if (strcmp(iface, "org.bluez.GattCharacteristic1")) {
		BT_SHELL_DBG("Unable to acquire notify: %s not a"
				" characteristic\n",
				g_dbus_proxy_get_path(proxy));
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
void gatt_release_notify(GDBusProxy *proxy, const char *arg)
{
	if (proxy != notify_io.proxy || !notify_io.io) {
		BT_SHELL_DBG("Notify not acquired\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
static void notify_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to %s notify: %s\n",
				enable ? "start" : "stop", error.name);
		dbus_error_free(&error);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	BT_SHELL_DBG("Notify %s\n", enable == TRUE ? "started" : "stopped");
...
void gatt_notify_attribute(GDBusProxy *proxy, bool enable)
{
	...
	BT_SHELL_DBG("Unable to notify attribute %s\n",
						g_dbus_proxy_get_path(proxy));
...
static void register_app_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to register application: %s\n",
				error.name);
		dbus_error_free(&error);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
					...
					int argc, char *argv[])
{
	...
	if (!l) {
		BT_SHELL_DBG("Unable to find GattManager proxy\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (uuids) {
		if (g_dbus_register_interface(conn, APP_PATH,
						...
						NULL) == FALSE) {
			BT_SHELL_DBG("Failed to register application"
					" object\n");
			return bt_shell_noninteractive_quit(EXIT_FAILURE);
		}
	}
	...
	if (g_dbus_proxy_method_call(l->data, "RegisterApplication",
						...
						NULL) == FALSE) {
		BT_SHELL_DBG("Failed register application\n");
		g_dbus_unregister_interface(conn, APP_PATH, PROFILE_INTERFACE);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
}
...
static void unregister_app_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to unregister application: %s\n",
				error.name);
		dbus_error_free(&error);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	BT_SHELL_DBG("Application unregistered\n");
...
void gatt_unregister_app(DBusConnection *conn, GDBusProxy *proxy)
{
	...
	if (g_dbus_proxy_method_call(l->data, "UnregisterApplication",
						...
						NULL) == FALSE) {
		BT_SHELL_DBG("Failed unregister profile\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
}
...
static void chrc_unregister(void *data)
{
	...
	PRINT_CHRC_DBG(chrc, COLORED_DEL);
			...
			void *data)
{
	...
	DBG_TR_LN("call print_service ... ["COLORED_CHG"]");
						...
						int argc, char *argv[])
{
	...
	if (g_dbus_register_interface(conn, service->path,
					...
					service_free) == FALSE) {
		BT_SHELL_DBG("Failed to register service object\n");
		service_free(service);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	DBG_TR_LN("call print_service ... ["COLORED_NEW"]");
						...
						int argc, char *argv[])
{
	...
	if (!service) {
		BT_SHELL_DBG("Failed to unregister service object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
					...
					int argc, char *argv[])
{
	...
	if (!local_services) {
		BT_SHELL_DBG("No service registered\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (!inc_service) {
		BT_SHELL_DBG("Failed to find  service object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
						...
						int argc, char *argv[])
{
	...
	service = service_find(argv[1]);
	if (!service) {
		BT_SHELL_DBG("Failed to unregister include service"
							" object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (!ser_inc) {
		BT_SHELL_DBG("Failed to find include service object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
			...
			void *data)
{
	...
	PRINT_CHRC_DBG(chrc, COLORED_CHG);
...
static DBusMessage *create_sock(struct chrc *chrc, DBusMessage *msg)
{
	...
	BT_SHELL_DBG("[" COLORED_CHG "] Attribute %s %s sock acquired\n",
							...
							void *user_data)
{
	...
	BT_SHELL_DBG("AcquireWrite: %s link %s\n", path_to_address(device),
									link);
							...
							void *user_data)
{
	...
	BT_SHELL_DBG("AcquireNotify: %s link %s\n", path_to_address(device),
									link);
							...
							void *user_data)
{
	...
	chrc->notifying = true;
	BT_SHELL_DBG("[" COLORED_CHG "] Attribute %s (%s) notifications "
							...
							void *user_data)
{
	...
	BT_SHELL_DBG("Attribute %s (%s) indication confirm received",
			chrc->path, bt_uuidstr_to_str(chrc->uuid));
...
void chrc_set_short(uint16_t value16, void *user_data)
{
	...
	{
		uint8_t values[10];
		DBG_WN_LN("(chrc: %p, chrc->uuid: %s, value16: %d)", chrc, chrc->uuid, value16);
		little_endian2byte(2, value16, (uint8_t*)values); // Message Type
#if GLIB_CHECK_VERSION(2, 68, 0)
		chrc->value = g_memdup2(values, sizeof(uint16_t));
#else
		chrc->value = g_memdup(values, sizeof(uint16_t));
#endif
	}
	...
	chrc->value_len = sizeof(uint16_t);
...
static void chrc_set_value(const char *input, void *user_data)
{
	...
	DBG_WN_LN("(chrc->uuid: %s, input: %s)", chrc->uuid, input);
					...
					int argc, char *argv[])
{
	...
	if (g_dbus_register_interface(conn, chrc->path, CHRC_INTERFACE,
		...
		BT_SHELL_DBG("Failed to register characteristic object\n");
		chrc_free(chrc);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	PRINT_CHRC_DBG(chrc, COLORED_NEW);
						...
						int argc, char *argv[])
{
	...
	if (!chrc) {
		BT_SHELL_DBG("Failed to unregister characteristic object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
							...
							void *user_data)
{
	...
	BT_SHELL_DBG("[%s (%s)] ReadValue: %s offset %u link %s\n",
			...
			void *data)
{
	...
	DBG_TR_LN("call print_desc ... ["COLORED_CHG"]");
...
static void desc_set_value(const char *input, void *user_data)
{
	...
	DBG_WN_LN("(desc->uuid: %s, input: %s)", desc->uuid, input);
						...
						int argc, char *argv[])
{
	...
	if (!service->chrcs) {
		BT_SHELL_DBG("No characteristic registered\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (g_dbus_register_interface(conn, desc->path, DESC_INTERFACE,
		...
		BT_SHELL_DBG("Failed to register descriptor object\n");
		desc_free(desc);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	DBG_TR_LN("call print_desc ... ["COLORED_NEW"]");
						...
						int argc, char *argv[])
{
	...
	if (!desc) {
		BT_SHELL_DBG("Failed to unregister descriptor object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
					...
					DBusMessageIter *iter, void *user_data)
{
	...
	BT_SHELL_DBG("[" COLORED_CHG "] Attribute %s (%s) %s:\n",
...
void gatt_clone_attribute(GDBusProxy *proxy, int argc, char *argv[])
{
	...
	if (argc > 1) {
		...
		if (!proxy) {
			BT_SHELL_DBG("Unable to find attribute %s\n",
								argv[1]);
			return bt_shell_noninteractive_quit(EXIT_FAILURE);
		}
	}
