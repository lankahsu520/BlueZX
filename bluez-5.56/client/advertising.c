...
{
	BT_SHELL_DBG("Advertising released\n");

	ad_release(conn);
#ifdef BLUEZX
	bluezx_advertising_released_cb();
#endif
...
static void register_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		ad.registered = true;
#ifdef BLUEZX
		bluezx_advertising_registered_cb();
#endif
		BT_SHELL_DBG("Advertising object registered\n");
		print_ad();
		/* Leave advertise running even on noninteractive mode */
	} else {
		BT_SHELL_DBG("Failed to register advertisement: %s\n", error.name);
		dbus_error_free(&error);

		if (g_dbus_unregister_interface(conn, AD_PATH,
						AD_IFACE) == FALSE)
			BT_SHELL_DBG("Failed to unregister advertising object\n");
...
{
	if (ad.registered) {
		BT_SHELL_DBG("Advertisement is already registered\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (g_dbus_register_interface(conn, AD_PATH, AD_IFACE, ad_methods,
		...
		BT_SHELL_DBG("Failed to register advertising object\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	if (g_dbus_proxy_method_call(manager, "RegisterAdvertisement",
					...
					conn, NULL) == FALSE) {
		BT_SHELL_DBG("Failed to register advertising\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
}
...
static void unregister_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		ad.registered = false;
#ifdef BLUEZX
		bluezx_advertising_unregistered_cb();
#endif
		BT_SHELL_DBG("Advertising object unregistered\n");
		if (g_dbus_unregister_interface(conn, AD_PATH,
							AD_IFACE) == FALSE)
			BT_SHELL_DBG("Failed to unregister advertising"
					" object\n");
		return bt_shell_noninteractive_quit(EXIT_SUCCESS);
	} else {
		BT_SHELL_DBG("Failed to unregister advertisement: %s\n",
								error.name);
		dbus_error_free(&error);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
void ad_unregister(DBusConnection *conn, GDBusProxy *manager)
{
	...
	if (g_dbus_proxy_method_call(manager, "UnregisterAdvertisement",
					...
					conn, NULL) == FALSE) {
		BT_SHELL_DBG("Failed to unregister advertisement method\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
}
...
void ad_advertise_service(DBusConnection *conn, int argc, char *argv[])
{
	...
	if (argc < 2 || !strlen(argv[1])) {
		if (ad.service.uuid) {
			DBG_IF_DUMP((char*)ad.service.data.data, ad.service.data.len, " ", "uuid(%s):", ad.service.uuid);
			print_uuid(ad.service.uuid);
...
void ad_advertise_manufacturer(DBusConnection *conn, int argc, char *argv[])
{
	...
	if (!endptr || *endptr != '\0' || val > UINT16_MAX) {
		BT_SHELL_DBG("Invalid manufacture id\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
void ad_advertise_secondary(DBusConnection *conn, const char *value)
{
	if (!value) {
		if (ad.secondary)
			 BT_SHELL_DBG("Secondary Channel: %s\n",
							ad.secondary);
		return bt_shell_noninteractive_quit(EXIT_SUCCESS);
	}
