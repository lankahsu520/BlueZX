					...
					DBusMessage *msg, void *user_data)
{
	...
	BT_SHELL_DBG("Request confirmation\n");
					...
					DBusMessage *msg, void *user_data)
{
	...
	BT_SHELL_DBG("Request authorization\n");
					...
					DBusMessage *msg, void *user_data)
{
	...
	BT_SHELL_DBG("Authorize service\n");
...
static void register_agent_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		...
		BT_SHELL_DBG("Agent registered\n");
#ifdef BLUEZX
		bluezx_agent_register_reply_cb(message, user_data);
#endif
	} else {
		BT_SHELL_DBG("Failed to register agent: %s\n", error.name);
		dbus_error_free(&error);

		if (g_dbus_unregister_interface(conn, AGENT_PATH,
						AGENT_INTERFACE) == FALSE)
			BT_SHELL_DBG("Failed to unregister agent object\n");
	}
}
...

{
	if (agent_registered == TRUE) {
		BT_SHELL_DBG("Agent is already registered\n");
		return;
	}
	...
	if (g_dbus_register_interface(conn, AGENT_PATH,
		...
		BT_SHELL_DBG("Failed to register agent object\n");
		return;
	}
	...
	if (g_dbus_proxy_method_call(manager, "RegisterAgent",
						...
						conn, NULL) == FALSE) {
		BT_SHELL_DBG("Failed to call register agent method\n");
		return;
	}
...
static void unregister_agent_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		BT_SHELL_DBG("Agent unregistered\n");
		agent_release(conn);
	} else {
		BT_SHELL_DBG("Failed to unregister agent: %s\n", error.name);
		dbus_error_free(&error);
	}
}
...
void agent_unregister(DBusConnection *conn, GDBusProxy *manager)
{
	if (agent_registered == FALSE) {
		BT_SHELL_DBG("No agent is registered\n");
		return;
	}
	...
	if (g_dbus_proxy_method_call(manager, "UnregisterAgent",
						...
						conn, NULL) == FALSE) {
		BT_SHELL_DBG("Failed to call unregister agent method\n");
		return;
	}
}
...
static void request_default_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		BT_SHELL_DBG("Failed to request default agent: %s\n",
							error.name);
		dbus_error_free(&error);
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	BT_SHELL_DBG("Default agent request successful\n");
...
void agent_default(DBusConnection *conn, GDBusProxy *manager)
{
	...
	if (g_dbus_proxy_method_call(manager, "RequestDefaultAgent",
						...
						NULL, NULL) == FALSE) {
		BT_SHELL_DBG("Failed to call RequestDefaultAgent method\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
}
