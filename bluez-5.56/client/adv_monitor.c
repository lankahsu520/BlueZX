					...
					DBusMessage *msg, void *user_data)
{
	...
	BT_SHELL_DBG("Advertisement monitor %d released\n",
							adv_monitor->idx);
					...
					DBusMessage *msg, void *user_data)
{
	...
	BT_SHELL_DBG("Advertisement monitor %d activated\n",
							adv_monitor->idx);
	return dbus_message_new_method_return(msg);
}
					...
					DBusMessage *msg, void *user_data)
{
	...
	dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &device,
							DBUS_TYPE_INVALID);
	BT_SHELL_DBG("Advertisement monitor %d found device %s\n",
					...
					DBusMessage *msg, void *user_data)
{
	...
	dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &device,
							DBUS_TYPE_INVALID);
	BT_SHELL_DBG("Advertisement monitor %d lost device %s\n",
						adv_monitor->idx, device);
	return dbus_message_new_method_return(msg);
}
...
void adv_monitor_add_manager(DBusConnection *conn, GDBusProxy *proxy)
{
	...
	if (manager.proxy != NULL || manager.supported_types != NULL ||
		...
		BT_SHELL_DBG("advertisement monitor manager already "
				"added\n");
		return;
	}
...
static void register_reply(DBusMessage *message, void *user_data)
{
	...
	if (!dbus_set_error_from_message(&error, message)) {
		BT_SHELL_DBG("AdvertisementMonitor path registered\n");
		return bt_shell_noninteractive_quit(EXIT_SUCCESS);
	}

	BT_SHELL_DBG("Failed to register path: %s\n", error.name);
...
static void unregister_reply(DBusMessage *message, void *user_data)
{
	...
	if (!dbus_set_error_from_message(&error, message)) {
		BT_SHELL_DBG("AdvertisementMonitor path unregistered\n");
		return bt_shell_noninteractive_quit(EXIT_SUCCESS);
	}

	BT_SHELL_DBG("Failed to unregister Advertisement Monitor:"
			" %s\n", error.name);
	dbus_error_free(&error);
	return bt_shell_noninteractive_quit(EXIT_FAILURE);
}
...
void adv_monitor_register_app(DBusConnection *conn)
{
	if (manager.app_registered) {
		BT_SHELL_DBG("Advertisement Monitor already registered\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	} else if (manager.supported_types == NULL ||
		!g_dbus_proxy_method_call(manager.proxy, "RegisterMonitor",
					register_setup, register_reply,
					NULL, NULL)) {
		BT_SHELL_DBG("Failed to register Advertisement Monitor\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
{
	if (!manager.app_registered) {
		BT_SHELL_DBG("Advertisement Monitor not registered\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
...
static struct pattern *parse_pattern(char *parameter_list[])
{
	...
	if (!pat) {
		BT_SHELL_DBG("Failed to allocate pattern\n");
...
static GSList *parse_patterns(char *pattern_list[], int num)
{
	...
	if (num == 0) {
		BT_SHELL_DBG("No pattern provided\n");
		return NULL;
	}

	if (num%3) {
		BT_SHELL_DBG("Expected %d more arguments\n", 3 - num%3);
		return NULL;
	}
...
static void print_adv_monitor(struct adv_monitor *adv_monitor)
{
	...
	BT_SHELL_DBG("Advertisement Monitor %d\n", adv_monitor->idx);
...
static struct rssi_setting *get_current_rssi(void)
{
	...
	if (!current_rssi)
		BT_SHELL_DBG("Failed to allocate rssi setting");
							...
							int argc, char *argv[])
{
	...
	if (g_slist_length(adv_mons) >= UINT8_MAX) {
		BT_SHELL_DBG("Number of advertisement monitor exceeds "
				"the limit");
		return;
	}
	...
	if (!adv_monitor) {
		BT_SHELL_DBG("Failed to allocate adv_monitor");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
	...
	BT_SHELL_DBG("Advertisement Monitor %d added\n", adv_monitor->idx);
}
...
void adv_monitor_print_monitor(DBusConnection *conn, int monitor_idx)
{
	...
	if (adv_monitor == NULL) {
		BT_SHELL_DBG("Can't find monitor with index %d\n",
								monitor_idx);
		return;
	}
...
void adv_monitor_remove_monitor(DBusConnection *conn, int monitor_idx)
{
	...
	remove_adv_monitor(adv_monitor, conn);
	BT_SHELL_DBG("Monitor %d deleted\n", monitor_idx);
}
...
{
	BT_SHELL_DBG("Supported Features:");
	print_supported_list(manager.supported_features);
	bt_shell_printf("\n");

	BT_SHELL_DBG("Supported Moniter Types:");
